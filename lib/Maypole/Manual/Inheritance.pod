...
L<Maypole> synopsis) consists of a custom driver class (BeerDB.pm), a set of
auto-generated model classes, and a view class:
...
Next, C<Maypole::setup> B<unshifts> L<Maypole::Model::CDBI> onto the C<@ISA> 
...
and C<BeerDB2::Maypole::Model> is B<unshifted> onto it's C<@ISA>.
...
Offline table classes, because C<BeerDB2::Maypole::Model> was unshifted onto the
beginning of each Maypole table class's C<@ISA>. Perl's depth first,
